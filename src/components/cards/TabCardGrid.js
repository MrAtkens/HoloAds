...
const CardContainer = tw.div`mt-10 w-full sm:w-1/2 md:w-1/2 lg:w-1/2 sm:pr-10 md:pr-6 lg:pr-12`;
      ...
      <Container>
        <ContentWithPaddingXl>
          <HeaderRow>
            ...
            <TabsControl>
              {data.map((item, index) => (
                  <TabControl key={index} active={activeTab === item.name} onClick={() => setActiveTab(item.name)}>
                    {item.name}
                  </TabControl>
              ))}
            </TabsControl>
