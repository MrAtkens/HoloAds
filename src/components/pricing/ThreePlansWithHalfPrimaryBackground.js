...
const Description = tw(SectionDescription)`w-full text-gray-500 text-center`;
...
const PlanFeatures = styled.ul`
  ...
  .feature {
    ...
    .icon {
      ${tw`w-6 h-6 text-orange-500 flex-shrink-0`}
    }
    .text {
...
const ActionButton = styled(PrimaryButtonBase)`
  ${tw`block text-center text-sm font-semibold tracking-wider w-full text-gray-100 bg-orange-600 px-6 py-4 rounded hover:bg-orange-700 focus:shadow-outline focus:outline-none transition-colors duration-300`}
`;
...
export default ({
    ...
    const defaultPlans = [
        ...
        },
        {
            ...
            price: ["$15", ".99/month"],
            oldPrice: "$19.99",
            description: "Perfect for hosting gallery with lots of traffic or heavy development projects",
            features: [
                "4 Core Xeon CPU",
                "2 GB RAM",
                "100 GB SSD",
        ...
        <Container>
            <ContentWithPaddingXl>
                <HeaderContainer>
                    ...
                    <Heading>{heading} <span style={{color: color}}>{secondHeading}</span></Heading>
